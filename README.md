...
Простой и эффективный прокси-сервер для Telegram Bot API, написанный на Node.js. Скрипт позволяет обходить сетевые ограничения, проксируя запросы к `api.telegram.org` через ваш собственный сервер с использованием HTTP/HTTPS прокси.

## 🚀 Возможности

- **Единый эндпоинт**: Обрабатывает любые методы Telegram API через путь `/bot<TOKEN>/<METHOD>`.
- **Поддержка HTTP-прокси**: Использует `https-proxy-agent` для перенаправления трафика.
